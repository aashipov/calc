...
pub fn viaMeval(expr: String) -> String {
    let expr_c_string = CString::new(expr).expect("CString::new failed");
    let expr_c_ptr: *const c_char = expr_c_string.as_ptr();
    unsafe {
        let result = via_meval(expr_c_ptr);
        return result.to_string();
    }
}

pub fn viaExprtk(expr: String) -> String {
    let expr_c_string = CString::new(expr).expect("CString::new failed");
    let expr_c_ptr: *const c_char = expr_c_string.as_ptr();
    unsafe {
        let result = via_exprtk(expr_c_ptr);
        return result.to_string();
    }
}
