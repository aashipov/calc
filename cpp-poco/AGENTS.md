# Calculator Service: POCO C++ flavor
...
- `src/CalcConstants.hpp`: constants file.
- `src/CalcRequestHandlerFactory.hpp`: a `Poco::Net::HTTPRequestHandlerFactory` implementation which handles `src/CalcGetHandler.hpp` and `src/CalcPostHandler.hpp` instantiation based on request method
...
- Modern `make`, `cmake`, `POCO`, `Google Test`
